function generate_train_data_attr {
    NAME="generate_train_data_attr"
    ...
    SELECT_ATTR_PATH="script/$SELECT_ATTR"
    ...
    hadoop jar /software/servers/hadoop-2.2.0/share/hadoop/tools/lib/hadoop-streaming-2.2.0.jar  \
               ...
               -input "${INPUT}" \
               -output "${OUTPUT}" \
               -mapper "python27/bin/python2.7 ${MAPPER} --category_attribute=$CATEGORY_ATTRIBUTE --lable_mapping=$LABEL_MAPPING --select_attr=$SELECT_ATTR" -file "${MAPPER_PATH}" -file "$CATEGORY_ATTRIBUTE_PATH" -file "$LABEL_MAPPING_PATH" -file "$SELECT_ATTR_PATH"
}
