function generate_train_data_attr {
    ...
    hadoop jar /software/servers/hadoop-2.2.0/share/hadoop/tools/lib/hadoop-streaming-2.2.0.jar  \
               ...
               -input "${INPUT}" \
               -output "${OUTPUT}" \
               -mapper "python27/bin/python2.7 ${MAPPER} --category_attribute=$CATEGORY_ATTRIBUTE --select_attr=$SELECT_ATTR" -file "${MAPPER_PATH}" -file "$CATEGORY_ATTRIBUTE_PATH" -file "$SELECT_ATTR_PATH"
}
