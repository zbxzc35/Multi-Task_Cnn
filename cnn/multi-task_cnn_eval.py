...
tf.app.flags.DEFINE_string('eval_data_dir', '/home/admin/zhexuanxu/multi-task_cnn_pairwise/data/test.bin',
                           """Either 'test' or 'train_eval'.""")
tf.app.flags.DEFINE_string('eval_batch_size', 1000,
...
def eval_once(saver, summary_writer, summary_op, prec, images, logits, labels, num_splits):
    """Run Eval once.
    Args:
      saver: Saver.
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            step = 0
            precisions, imag, logi, labe, nsplit = sess.run([prec, images, logits, labels, num_splits])
            precisions = [precisions]
            #record_wrong_predict(imag, logi, labe, nsplit)
            #print precisions
            while step < num_iter and not coord.should_stop():
                p, imag, logi, labe, nsplit = sess.run([prec, images, logits, labels, num_splits])
                #record_wrong_predict(imag, logi, labe, nsplit)
            ...
            # Compute precision @ 1.
            print('{}: precision mean{} @ 1 = {}'.format(datetime.now(), np.mean(precisions),precisions))
...
def evaluate():
    ...
    with tf.Graph().as_default() as g:
        ...
        images, skuid, labels, hots = cnn.inputs(FLAGS.eval_data_dir, eval_data=True, batch_size=FLAGS.eval_batch_size)
        ...
        while True:
            eval_once(saver, summary_writer, summary_op, prec, images, logits, labels, num_splits)
            if FLAGS.run_once:
                break
