...
tf.app.flags.DEFINE_string('eval_batch_size', 1,
...
def calcuate_prediction(logits, labels, num_splits):
    ...
    logit_argmax = tf.argmax(logits_split[0], 1)
    labels_argmax = tf.argmax(labels_split[0], 1)
    for i in range(1, len(labels_split)):
        ...
        logit_argmax = tf.concat([logit_argmax, tf.argmax(logits_split[i], 1)], axis=0)
        labels_argmax = tf.concat([labels_argmax, tf.argmax(labels_split[i], 1)], axis=0)
    #tf.summary.histogram("prediction", pred) 

    return pred, logit_argmax, labels_argmax    

def eval_once(saver, summary_writer, summary_op, pred, logits, labels, hots, logl, labl):
    """Run Eval once.
    Args:
      saver: Saver.
        ...
        try:
            ...
            with open('test', 'wb') as f:
                while step < num_iter and not coord.should_stop():
                    p, logll, labll, logitss, labelss, hotss = sess.run([pred, logl, labl, logits, labels, hots])
                    f.write('{}\n'.format(p))
                    f.write('{}\n'.format(logll))
                    f.write('{}\n'.format(labll))
                    #logitss = sess.run(logits)
                    #labelss = sess.run(labels)
                    #hotss = sess.run(hots)
                    for x in logitss[0]:
                        f.write('{}\t'.format(x))
                    f.write('\n')    
...
def evaluate():
    ...
    with tf.Graph().as_default() as g:
        ...
        pred, logl, labl = calcuate_prediction(logits, labels, num_splits)
        ...
        while True:
            eval_once(saver, summary_writer, summary_op, pred, logits, labels, hots, logl, labl)
            if FLAGS.run_once:
                break
